...
class Prediction:
  ...
  def predict(self, user_id: int, movie_id: int) -> float:
    ...
    if user_k_nearest_neighbours.empty:
      return 0.0
    return self.__calculate_neighbour_weighted_avg_rating(movie_id, user_id, user_k_nearest_neighbours)
  ...
  def __calculate_neighbour_weighted_avg_rating(self, movie_id, user_id, user_k_nearest_neighbours) -> float:
    avg_user_rating = self.__dataset_user_operator.get_user_avg(user_id)
    sum_of_weights, weighted_sum = self.__take_weighted_neighbour_rating_average(movie_id, user_k_nearest_neighbours)
    return avg_user_rating + (weighted_sum / sum_of_weights) if sum_of_weights != 0 else 0

  def __take_weighted_neighbour_rating_average(self, movie_id, user_k_nearest_neighbours) -> (float, float):
    ...
    for neighbour_id, neighbour in user_k_nearest_neighbours.iterrows():
      ...
      if neighbour_rating != 0:
        ...
        weighted_sum += neighbour_zero_centered_rating * neighbour['correlation']
        sum_of_weights += neighbour['correlation']
